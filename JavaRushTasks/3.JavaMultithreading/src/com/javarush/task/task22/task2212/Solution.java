...
public class Solution {
    public static boolean checkTelNumber(String telNumber) {
        if (telNumber == null || telNumber.equals(""))
            return false;
        boolean isPlus = telNumber.substring(0, 1).equals("+");
        boolean isBrackets = telNumber.contains("(") && telNumber.contains(")");
        boolean isDash = telNumber.contains("-");

        if (!isPlus && !isBrackets && !isDash)
            return telNumber.matches("\\d{10}");
        else if (isPlus && !isBrackets && !isDash)
            return telNumber.matches("\\+\\d{12}");
        else if (!isPlus && isBrackets && !isDash)
                return telNumber.matches("\\d*\\(\\d{3}\\)\\d+") && telNumber.length() == 12;
        else if (isPlus && isBrackets && !isDash)
                return telNumber.matches("\\+\\d*\\(\\d{3}\\)\\d+") && telNumber.length() == 15;
        else if (!isPlus && !isBrackets)
            return telNumber.matches("\\d+-\\d+") && telNumber.length() == 11 || telNumber.matches("\\d+-\\d+-\\d+")  && telNumber.length() == 12;

        else if (isPlus && !isBrackets)
            return telNumber.matches("\\+\\d*-\\d+") && telNumber.length() == 14 || telNumber.matches("\\+\\d*-\\d+-\\d+")  && telNumber.length() == 15;
        else if (!isPlus)
                return telNumber.matches("\\d*\\(\\d{3}\\)\\d*-\\d+") && telNumber.length() == 13 || telNumber.matches("\\d*\\(\\d{3}\\)\\d*-\\d+-\\d+") && telNumber.length() == 14;
        else return telNumber.matches("\\+\\d*\\(\\d{3}\\)\\d*-\\d+") && telNumber.length() == 16 || telNumber.matches("\\+\\d*\\(\\d{3}\\)\\d*-\\d+-\\d+") && telNumber.length() == 17;
    }
